...
const MessManagerRequests = () => {
  ...
  const handleApprove = async (id) => {
    try {
      ...
      setRequests(prev =>
        prev.map(r =>
          r._id === id ? { ...r, messManagerApproval: true, status: 'Approved' } : r
        )
      );
        ...
        <tbody>
          {fields.map(([label, value], i) => (
            <tr key={i}>
              ...
              <td>{value}</td>
              {i === 0 && (
                <td rowSpan="11" style={{ width: '160px' }} className="text-center align-middle">
                  <Button
                    color="primary"
                  ...
                  <Button
                    ...
                    disabled={student.messManagerApproval}
                  >
                    {student.messManagerApproval ? 'Approved' : 'Approve'}
                  </Button>
                </td>
              )}
            </tr>
  ...
  return (
    <div className="p-4">
      <h3 className="mb-4">Mess Deduction Requests (Mess_Manager Approved)</h3>

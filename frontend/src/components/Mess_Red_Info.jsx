...
const MessReduction_info = () => {
  ...
  useEffect(() => {
    const fetchMessData = async () => {
      try {
        setLoading(true);
        const response = await fetch('http://localhost:8080/messdeduction/all');
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        
        const result = await response.json();
        const dataArray = Array.isArray(result) ? result : 
                         result.documents || result.data || [];
        setMessData(dataArray);
      } catch (err) {
      ...
      if (response.ok) {
        setMessData(prev => prev.map(student => 
          student._id === studentId 
            ? { ...student, status: 'Approved', wardenApproval: true }
            : student
        ));
  ...
  const handleRejectWithReason = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setMessData(prev => prev.map(s => 
          s._id === student._id 
            ? { ...s, status: 'Rejected', wardenApproval: false, rejectionReason: reason.trim() }
            : s
        ));
    ...
    <div className={isMobile ? 'p-2 mobile-style' : 'p-4 desktop-style'}>
      ...
      {messData.length === 0 ? (
        <div className="alert alert-info">
          <p>No mess reduction applications found.</p>
        </div>
      ) : (
            ...
            {preview.student && (
              <>
                <Button 
                  ...
                  disabled={preview.student.status === 'Approved'}
                >
                  {preview.student.status === 'Approved' ? 'Already Approved' : 'Approve'}
                </Button>
                <Button 
                  color="danger" 
                  onClick={() => { 
                  ...
                  disabled={preview.student.status === 'Rejected'}
                >
                  {preview.student.status === 'Rejected' ? 'Already Rejected' : 'Reject'}
                </Button>
              </>
            )}
          </>
